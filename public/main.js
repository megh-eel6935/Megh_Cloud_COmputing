 ...
 var urlRegex =/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig;
  ...
  function updategrouplist(){
    $.get( "/groupslist", function( data ) {
      sap = JSON.stringify(data);
      var length = data.data.length;
      var temp ="<li class=\"list-group-item groupclick\" id="+data.data[length-1].groupid+" align=\"center\"><span id=\"badge"+data.data[length-1].groupid+"\" class=\"badge\"></span> "+data.data[length-1].groupname+"</li>";
      //var temp = "<option value="+data.data[i].groupid+">"+data.data[i].groupname+"</option>";
      $("#addgroups").append(temp);
    });
  }
